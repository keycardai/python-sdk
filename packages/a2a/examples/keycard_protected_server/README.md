...
- `KeycardServerCallContextBuilder()`: propagates the verified `KeycardUser` into a2a-sdk's `ServerCallContext.state`; the executor reads it back with `keycard_user(context)` and uses `.access_token` for delegated downstream calls.
